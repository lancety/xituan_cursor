...
description: >-
  ...
  codebase submodule, full-repo commit, multirepo sync, align all projects after shared typing/util changes,
  or push Cursor rules/skills/plans (xituan_cursor).
---

# Xituan multirepo: xituan_codebase + main projects + agent + cursor
...
| 路径 | 远程仓库 | 说明 |
|------|----------|------|
| `xituan_backend/submodules/xituan_codebase` | `lancety/xituan_codebase` | 共享 codebase：**流程起点**，先于此完成 pull / 提交 / 推送 |
| `xituan_backend` | `lancety/xituan_backend` | 主项目 + submodule |
| `xituan_cms` | `lancety/xituan_cms` | 主项目 + submodule |
...
各主项目 `.gitignore` 常忽略 `.cursor/`；`.cursor` 为**独立 Git 根**，与主项目提交互不影响。
...
本技能流程内任意步骤产生的 **`git commit`**（阶段 1 `xituan_codebase`、阶段 2 consumer 子模块、阶段 3 主仓、阶段 4 `xituan_agent`、阶段 5 `.cursor` / `xituan_cursor`）之 **message 正文**：
...
**可选跳过（与主仓「无待同步」同逻辑）**：在 `xituan_agent` 根目录若同时满足 — 当前为 `master`、工作区干净、`git fetch origin master` 后 `HEAD` 与 `origin/master` 一致 — 则**跳过**本阶段 `pull` / `commit` / `push`（除非用户要求强制全量）。
...
若 `pull` 前工作区非空，按 **「Stash 使用规则」** 使用 `$STASH_TAG`：`stash push` → `pull` → `stash pop "stash^{/$STASH_TAG}"`（仅当本目录本轮确实创建过 stash）。

---

### 阶段 5 — `xituan_cursor`（`.cursor`，整链最后）

目录：`.cursor`（Git 根；远程 `https://github.com/lancety/xituan_cursor.git`）

**内容范围**：`rules/`、`skills/`、`plans/` 及仓库内 `.gitignore`、`README.md` 等；**不要**把各主项目业务代码提交进此仓库。

**先做「`master` 分支与终止条件」检查**；通过后：

**可选跳过**：在 `.cursor` 根目录若同时满足 — 当前为 `master`、工作区干净、`git fetch origin master` 后 `HEAD` 与 `origin/master` 一致 — 则**跳过**本阶段（除非用户要求强制全量）。
...
git pull origin master
git add -A
git commit -m "chore(cursor): concise summary"   # only if changes; e.g. rules/skills/plans updates
git push origin master
```

若 `pull` 前工作区非空，按 **「Stash 使用规则」** 使用**新的** `$STASH_TAG`（若阶段 4 已用过 stash，此处须重新生成随机 tag）：`stash push -u -m "$STASH_TAG"` → `pull` → `stash pop "stash^{/$STASH_TAG}"`（仅当本目录本轮确实创建过 stash）。

**Commit message 示例**：`chore(cursor): add platform metadata migration plan`、`docs(skills): extend multirepo sync for xituan_cursor`。
